...
This helps to limit the memory footprint for garbling and evaluation required for storing circuit description and wire keys ($k$-bit per wire, see \sect{subsec:preli_GC}).
...
To do so, we add the memory element of sequential circuits into the technology library of TinyGarble (described in \sec{sec:syn_tech}).

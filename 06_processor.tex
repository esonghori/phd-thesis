...
A version of \sect{sec:processor-pfsfe} and \sect{sec:processor-pro-pfsfe} of this chapter has been published in 2015 IEEE Symposium on Security and Privacy (S\&P) \cite{songhori2015tinygarble}.
...
Two-party \acrfull{pf-sfe} allows secure computation of a function $g_{Alice}(\cdot)$ held by one party (Alice) operating on another party's data $a_{Bob}$ (Bob) while both the data and the function are kept private.
...
$$\acrshort{uc}(g_{Alice}(\cdot),a_{Bob}) = g_{Alice}(a_{Bob}).$$
...
A \acrshort{uc} is similar to a \acrfull{utm} \cite{turing1936computable,herken1995universal} that receives a Turing machine description $f_{Alice}(\cdot)$ and applies it to the input data ($a_{Bob}$) on its tape \cite{davis2001engines}.
One party provides the machine description and the other one provides the initial data.
The output $g_{Alice}(a_{Bob})$ resides on the tape after the operation is completed.
A general purpose processor is a special realization of a \acrshort{utm}.
It receives a list of \emph{instructions} $g_{Alice}(\cdot)$ (the compiled binary code of $g_{Alice}(\cdot)$) and applies them to the input data $a_{Bob}$.
...
We assume Alice provides the private function $g_{Alice}(\cdot)$ and Bob provides private data $a_{Bob}$.
At the end of the operation, only Bob learns the output $g_{Alice}(a_{Bob})$.
...
\item
  Alice reveals the output types such that Bob learns the value of the output $g_{Alice}(a_{Bob})$ stored in memory.
...
In the standard 2-party \acrshort{pf-sfe}, Alice provides the function $g_{Alice}(\cdot)$ and Bob provides the input data $g_{Bob}$ and the output is $g_{Alice}(a_{Bob})$.
Similar to \sect{sec:processor-pro-pfsfe}, the garbled processor receives a list of \emph{instructions} of compiled $g_{Alice}(\cdot)$ and applies them to the input data $a_{Bob}$ in memory and the output will be written back to the memory.

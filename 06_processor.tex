...
Then, we explain how \gls{arm}, a more sophisticated processor, can be well suited for solving \acrshort{sfe} development if \gls{skipgate} algorithm is applied during garbling/evaluating its circuit.
...
\subsection{\gls{mips}}\label{ssec:processor-mips}
...
Any arbitrary C/C++ function can be easily compiled to \gls{mips}~I assembly code using a cross-platform compiler e.g., GNU gcc.

In 32-bit \gls{mips}, the \acrfull{pc} is a 32-bit register (array of \acrshort{ff}s) that points to the instruction being executed at the current cycle.
...
(1) \acrfull{sscd} memory: read-only memory that stores the information about gates in the \gls{mips} circuit in \acrshort{sscd} format (see \appx{sec:engine-sscd})
...
The \gls{arm2gc} framework allows users to write two-party \acrshort{sfe} program in C/C++ (or any language that can be compiled to \gls{arm} binary code).
\fig{fig:frwk_overview} shows the overview of the framework.
The framework benefits from the \gls{skipgate} algorithm to reduce the cost of garbling the ARM processor.
As described in \chap{chap:skipgate}, the \gls{skipgate} algorithm supports secure evaluation of circuits in the form of $f(a,b,p)$ where $a$ and $b$ are Alice's and Bob's inputs and $p$ is a public input.
In the \gls{arm2gc} framework, the circuit $f(\cdot,\cdot,\cdot)$ is the circuit of ARM processor and the public input $p$ is the compiled binary code of the \acrshort{sfe} program.
...
The \gls{arm2gc} framework supports the following \acrshort{api}:
...
The circuit of our \gls{arm} processor has five separate memory elements (consisting of flip-flops and \acrshort{mux}s) to store: Alice's inputs, Bob's inputs, output, stack, and instructions.
...
\subsection{\gls{arm} as a Garbled Processor}\label{ssec:arm}
...
Implementation of \gls{arm} processor results in a complex and large netlist ($\approx 5$ times larger than \gls{mips} processor).
...
According to \gls{skipgate}'s notation, the \gls{arm} Boolean circuit is a 3-input function $c = f(a,b,p)$ where $p$ is the public binary code of $g(a,b)$ and $a$ and $b$ are the parties' private inputs.
\gls{skipgate} reduces the \gls{arm} circuit into a smaller circuit of $c = f_p(a,b)$ where $f_p$ is able to perform the exact operation required in the public binary code $p = g(\cdot,\cdot)$.
...
As mentioned in \sect{ssec:arm-global}, we use an array of \acrshort{mux}s and flip-flops to implement the register file in \gls{arm} circuit.

...
A \acrshort{uc} is similar to a Universal Turing Machine (UTM) \cite{turing1936computable,herken1995universal} that receives a Turing machine description $f_{Alice}(\cdot)$ and applies it to the input data ($x_{Bob}$) on its tape \cite{davis2001engines}.
...
\item
  Using any secure \acrshort{gc} framework, Alice garbles the processor circuit for the maximum number of sequential cycles and Bob, after receiving his inputs with OT, evaluates the garbled processor circuit for the same number of cycles.
...
\acrshort{mips} is a text-book Reduced Instruction Set Computing (RISC) \acrshort{isa} \cite{kane1992mips}.
The RISC \acrshort{isa} consists of a small set of simplified assembly instructions in contrast to Complex Instruction Set Computing (CISC) (e.g., x86 \acrshort{isa}) which includes more complex multi-step instructions \cite{hennessy2012computer}.
We choose a RISC \acrshort{isa} processor instead of CISC for the following main reasons: (i) lower number of non-XOR gates, (ii) simple and straightforward implementation, and (iii) availability and diversity of open-source implementations.
...
Bob's outputs are connected to the outputs of \acrshort{ff}s in the specified address of DM.
...
In 32-bit \acrshort{mips}, the program counter (\emph{PC}) is a 32-bit register (array of \acrshort{ff}s) that points to the instruction being executed at the current cycle.
...
The output of \acrshort{alu} will be either written back to the register file or fed to DM as an address for load/store.
...
(1) Sequential Simple Circuit Description (SSCD) memory: read-only memory that stores the information about gates in the \acrshort{mips} circuit in SSCD format (see \appx{sec:engine-sscd})
...
(3) Garbled Tables (GT) memory: read-write random-access memory that stores the ciphertext garbled tables of each non-XOR gate in the \acrshort{mips} circuit that are generated by Alice (garbler).
...
\caption{Our \acrshort{gc} Evaluator Architecture.}
\label{fig:evaluator}
\end{figure}
...
\subsection{Extending Hardware Prototype} \label{ssec:processor-hardware-extend}
...
The \acrshort{sfe} program is compiled using an \acrshort{arm} cross-compiler, e.g., gcc-arm-linux-gnueabi.
The SkipGate algorithm supports secure evaluation of function in the form of $f(a,b,p)$ where $a$ and $b$ are Alice's and Bob's inputs and $p$ is public input.
In ARM2GC, the compiled binary code and the synthesized \acrshort{arm} processor circuit are fed to the SkipGate algorithm as the public input ($p$) and the Boolean circuit ($f(\cdot,\cdot,\cdot)$), respectively.
...
The \acrshort{arm} circuit is garbled using sequential garbling process of TinyGarble GC engine (see \appx{chap:engine}) for a pre-specified number of sequential cycles $cc$.
...
According to SkipGate's notation, the \acrshort{arm} Boolean circuit is a 3-input function $c = f(a,b,p)$ where $p$ is the public binary code and $a$ and $b$ are the parties' private inputs.
SkipGate reduces the \acrshort{arm} circuit into a smaller circuit of $c = f_p(a,b)$ where $f_p$ is able to perform the exact operation required in the public binary code $p$, e.g., for example $c = a + b$.

...
Supporting sequential circuits in the GC protocol enables us to securely evaluate a general processor as function in two-party \acrshort{sfe}.
...
Next, we explain how we used MIPS processor, a simple text-book processor, to solve \acrshort{pf-sfe} problem.
...
Then, we explain how ARM, a more sophisticated processor, can be well suited for solving \acrshort{sfe} development if SkipGate algorithm is applied during garbling/evaluating its circuit.
...
It is well known that \acrshort{pf-sfe} can be reduced to regular \acrshort{sfe} by securely evaluating a Universal Circuit (UC) \cite{sander1999non}.
...
Subsequent works have shown how to allow \acrshort{pf-sfe} while avoiding the overhead of UCs \cite{katz2011constant, mohassel2013hide}.
...
However, one can simply connect our implementation of \acrshort{pf-sfe} to an ORAM to benefit from its lower amortized complexity.
...
Besides enabling \acrshort{pf-sfe}, another advantage of a garbled processor is usability for non-expert users since it can be programmed using high-level languages, whereas other frameworks for the GC protocol require tedious Boolean circuit construction.
...
In this section, we expand the garbled processor introduced in \section{sec:processor-pro-pfsfe} and introduce a framework for secure computation that provides scalable support for generalized \acrshort{sfe}.
...
To avoid information leakage about the function (i.e., \acrshort{pf-sfe}), we employ the MIPS circuit with its full Instruction Set (IS), which incurs a large overhead due to garbling and evaluating of the entire IS.
...
Using a general-purpose processor with its entire IS in \acrshort{sfe} results in garbling a large processor which is very costly and unnecessary since both parties know the function instructions being executed but not their results.
...
\section{ARM2GC: Garbled ARM for \acrshort{sfe}}
...
The ARM2GC framework allows users to write two-party \acrshort{sfe} program in C/C++ (or any language that can be compiled to ARM binary code).
\fig{fig:frwk_overview} shows the overview of the framework.
The \acrshort{sfe} program is compiled using an ARM cross-compiler, e.g., gcc-arm-linux-gnueabi.

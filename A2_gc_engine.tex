...
First, we explain implementation of sequential \acrshort{gc} protocol in \gls{tinygarble} \acrshort{gc} engine.
...
Besides clock (clk) and reset (rst) ports that are not relevant in the \acrshort{gc} protocol, flip-flops has three other ports: initial (I), input (D), output (Q).
...
In the \acrshort{sscd} format, we index wires according the following order: (1) g\_init (2) e\_init (3) g\_input (4) e\_input (5) gates' output.

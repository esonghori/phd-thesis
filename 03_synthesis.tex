...
Next, we explain TinyGarble \acrshort{gc} synthesis flow that generates optimized combinational circuits for the \acrshort{gc} protocol using \acrshort{hdl} synthesis in .
...
Classic commercial/open-source \acrshort{hdl} synthesis tools accept the input functions in the \acrshort{hdl} format, e.g., Verilog or VHDL \cite{tool:DesignCompiler,tool:ABC,tool:Encounter,tool:HDLdesigner,tool:PandA,decaluwe2004myhdl} but newer ones also accept high level format, e.g., C/C++ \cite{Gupta2004, tool:Vivado}.
...
Newer generations of synthesis programs, referred to as high level synthesis (\acrshort{hls}) tools, accept other forms of input in a higher level programming language \cite{Chapter:Zhang2008,chu1989hyper,corazao1996performance}, e.g., ANSI C, C++, SystemC, or Python.
...
\section{TinyGarble \acrshort{hdl} Synthesis}\label{sec:syn-tiny}
...
In TinyGarble framework, the user may describe a function in a standard \acrshort{hdl} like Verilog or VHDL.
...
TinyGarble uses existing \acrshort{hdl} synthesis tools to map an \acrshort{hdl} to a list of basic binary gates.
...
The global flow of TinyGarble \acrshort{gc} synthesis is shown in \fig{fig:synthesis-flow}.
It consists of the following four steps:

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{synthesis_flow-crop.pdf}
\caption{TinyGarble \acrshort{gc} synthesis flow.
...
\begin{enumerate}
\item The input to TinyGarble \acrshort{gc} synthesis is a file that describes a function written in an \acrshort{hdl} like Verilog or VHDL.
...
\item The netlist is parsed and topologically sorted.
      Then, the sorted netlist is stored in format readable by TinyGarble \acrshort{gc} engine.
\end{enumerate}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{HLS_HDL_netlist-crop.pdf}
\caption{Sample files at the different steps of TinyGarble \acrshort{gc} synthesis flow for Hamming distance function.}
\label{fig:globalflow_sample}
\end{figure}

\fig{fig:globalflow_sample} shows examples of files at different steps of TinyGarble \acrshort{gc} synthesis flow for Hamming distance function.
...
The \textsl{hamming.v} file is provided to an \acrshort{hdl} synthesis tool along with the TinyGarble custom libraries to generate netlist \textsl{hamming\_netlist.v}.
...
The technology library contains 2-input-1-output logic cells to be compatible with TinyGarble \acrshort{gc} engine.
...
For example, tools targeting Field Programmable Gate Arrays (FPGAs) like Xilinx ISE or Quartus contain Look-Up Tables and Flip Flops in their technology libraries, which form the architecture of an \acrshort{fpga}.
...
For \acrshort{hdl} synthesis, we use tools targeting ASICs as they allow more flexibility in their input technology library.
...
In TinyGarble \acrshort{gc} synthesis, we use \acrshort{hdl} synthesis tools in an offline manner to generate a circuit for a given functionality.
This offline synthesis followed by a topological sort provides a ready-to-use circuit description for any \acrshort{gc} implementations including our TinyGarble \acrshort{gc} engine.

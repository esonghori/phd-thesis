...
\item A standard logic synthesis tool compiles the \acrshort{hdl} to generate the \gls{netlist} file.
      ...
      The contains and library are set such that the \gls{netlist} is optimized for evaluation in the \acrshort{gc} protocol.

\item The \gls{netlist} is parsed and topologically sorted.
      Then, the sorted \gls{netlist} is stored in format readable by \gls{tinygarble} \acrshort{gc} engine.
\end{enumerate}
...
A user can also write the description directly in \gls{verilog} to have more control on the circuit and therefore a more efficient \gls{netlist}.
The \textsl{hamming.v} file is provided to a logic synthesis tool along with the \gls{tinygarble} custom libraries to generate \gls{netlist} \textsl{hamming\_netlist.v}.
...
By choosing area minimization as the only optimization goal, the synthesis tool produces a \gls{netlist} with the minimum possible number of non-XOR gates.

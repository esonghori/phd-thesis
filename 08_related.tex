...
The main vulnerability that a malicious adversary can use to gain information about the other party's input is generation of a faulty garbled circuits by Alice.
A straightforward method to make the protocol secure against a malicious adversary is to apply Goldreich-Micali-Wigderson (GMW) compiler to the protocol \cite{goldreich1987play}, such that Alice can prove to Bob that she garbled the circuit correctly without letting Bob knows the details.
...
Lindell and Pinkas (2007) proposed the first practical and efficient method to secure the \acrshort{gc} protocols against malicious adversaries in \cite{lindell2007efficient}.
They propose to use a cut-and-choose technique to do so.
Alice (garbler) using the original \acrshort{gc} protocol (\acrshort{hbc} adversary) garbles the circuit for multiple times (a statistical security
parameter) and send them to Bob (evaluator).
...
This method creates an other security challenge that is to make sure that the parties are using the same inputs for the multiple invocations of the garbled circuit.
To solve this problem, the authors propose to use a commitment schemes for input such that parties cannot change their input after the commitment.
The author later improve their method to make it more efficient in \cite{lindell2012secure}.

Securing \acrshort{gc} using this method incurs a multiplicative factor (the statistical security parameter) to the cost of \acrshort{gc} compared to the \acrshort{hbc} model.
Thus, reducing the cost of \acrshort{gc} under \acrshort{hbc} (one of the objectives of this thesis) results in an immediate reduction in the cost of \acrshort{gc} under malicious adversary.

Since in this thesis, we use the \acrshort{gc} protocol as a black-box, one can easily apply the methods proposed by Lindell and Pinkas \cite{lindell2007efficient, lindell2012secure} to secure our frameworks against a malicious adversary.
For example, in the \gls{arm2gc} framework described in \sect{sec:processor-arm}, the Boolean circuit is the circuit of an \gls{arm} processor and the inputs are parties private inputs.
Using the \gls{skipgate} algorithm, both parties agree on a simplified Boolean circuit of the \gls{arm} processor that is going to be garbled/evaluated.
Using the method of Lindell and Pinkas, Alice and Bob first commit their inputs.
Then, Alice generates multiple garbled circuits from the simplified \gls{arm} circuit and sends them to Bob.
...
Instead of asking to open the entire circuit, Bob asks her to open half of the NAND gates.
...
The authors showed that their method for a large circuit is more efficient compared to \cite{lindell2007efficient}.
Since in this thesis, we focus on generating a Boolean function with the minimum number of non-XOR gates, LEGO cannot directly enjoy from the improvement of \gls{tinygarble} synthesis similar to the work by Lindell and Pinkas.

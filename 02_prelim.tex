...
In the following, we explain how the problem of two-party \acrshort{sfe} can be solved using Yao's Garbled Circuit (\acrshort{gc}) protocol.
...
\subsection{Recent Improvements on the \acrshort{gc} Protocol}\label{ssec:prelim-imp}
...
In a number of \acrshort{sfe} protocols including Yao's \acrshort{gc}, Goldreich-Micali-Wigderson (GMW)~\cite{goldreich1987play}, and Beaver-Micali-Rogoway (BMR)~\cite{beaver1990round}, the function has to be represented as a Boolean circuit.
...
In this thesis, we use only D-type \acrshort{ff} shown in \fig{fig:ff}.
This type of \acrshort{ff} has four inputs: D (data), I (initial), clk (clock), and rst (reset) and one output: Q (latched data).

\begin{figure}[ht]
    \centering
	\includegraphics[width=0.35\textwidth]{ff-crop.pdf}
	\caption{Schematic of a D-\acrshort{ff} with initial value.}
	\label{fig:ff}
\end{figure}
...
In TinyGarble, signal I of an \acrshort{ff} can be connected to a constant value or input wire (in this cased called \emph{init} wire).

\chapter{TinyGarble \acrshort{gc} Engine}\label{chap:engine}
In this section, we explain TinyGarble \acrshort{gc} Engine, an implementation of the Yao's \acrshort{gc} protocol for sequential circuits.
First, we explain JustGarble \cite{bellare2013efficient}, and earlier \acrshort{gc} engine which TinyGarble is built on. \TODO{Do we need this here?}
...
We implemented the \acrshort{gc} engine in C++ and its source code is available in TinyGarble repository\footnote{\url{https://github.com/esonghori/TinyGarble}}.

\section{Yao's \acrshort{gc} Protocol Implementation for Sequential Circuits} \label{sec:engine-gc}
TinyGarble \acrshort{gc} engine is an implementation of Yao's protocol for garbling and evaluation of sequential circuits.
...
One of the prominent feature of TinyGarble \acrshort{gc} engine is its support for garbling/evaluation of sequential circuits.
...
As in previous work, security and correctness of the \acrshort{gc} garbling/evaluation follow from the uniqueness of the tweak $T$ and the existing proofs of security and correctness are still valid for TinyGarble (see \cite{lindell2009proof, bellare2013efficient, zahur2015two}).
...
The netlist format needs to be translated into SCD format to be readable by TinyGarble \acrshort{gc} engine.
...
The communication and computation in TinyGarble \acrshort{gc} engine follows the steps below:
\begin{enumerate}
...
The communication and computation in TinyGarble \acrshort{gc} engine for sequential circuit in low memory mode follows the steps below:
...
TinyGarble \acrshort{gc} engine supports the SkipGate algorithm on top of its implementation of the \acrshort{gc} protocol.
The code of the engine is changed according to the SkipGate algorithm described in \chap{chap:skipgate}.
